...
def load_response():
    global responses
    responses = {}
    with open('./dataset/dataset_wibu.json', encoding='utf-8') as content:
...
def preparation():
    ...
    tokenizer = pickle.load(open('model/baru/tokenizers.pkl', 'rb'))
    le = pickle.load(open('model/baru/le.pkl', 'rb'))
    model = keras.models.load_model('model/baru/chat_model.h5')
...
def remove_punctuation(text):
    texts_p = []
    text = [letters.lower()
            for letters in text if letters not in string.punctuation]
    text = ''.join(text)
    texts_p.append(text)
    return texts_p
